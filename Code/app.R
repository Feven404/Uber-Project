...
combined_data <- readRDS("Combined Data.rds")
...
heatmap_of_trips_by_hour_day <- readRDS("Heatmap of Trips by Hour and Day.rds")
...
server <- function(input, output) {
  ...
  output$plot2 <- renderPlot({
    ggplot(trips_by_every_hour, aes(x = Hour, y = Trips)) +
